...
public class BattleTest {
    ...
    public void assertBattleCalculations(
        ...
        double hydraIncrease = 0.0;
        if (enemyType.equals("hydra")) {
            hydraIncrease = Double
                    .parseDouble(
                            TestUtils.getValueFromConfigFile(enemyType + "_health_increase_amount", configFilePath));
        }

        for (RoundResponse round : rounds) {
            assertEquals(-enemyAttack / 10, round.getDeltaCharacterHealth(), 0.001);
            if (enemyType.equals("hydra")) {
                if (round.getDeltaEnemyHealth() != hydraIncrease) {
    ...
    public boolean compareHydraPreviousHealth(BattleResponse battle, String configFilePath, String entityType) {
        // stays true if hydra the hydra doesn't lose health
        Boolean condition = true;

        List<RoundResponse> rounds = battle.getRounds();
        double enemyHealth = battle.getInitialEnemyHealth(); // Should come from config
        double previousHealth = enemyHealth;
        double hydraIncrease = Double
                .parseDouble(
                        TestUtils.getValueFromConfigFile(entityType + "_health_increase_amount", configFilePath));
        ...
        for (RoundResponse round : rounds) {
            ...
            if (enemyHealth < previousHealth) {
                ...
                equals(enemyHealth = previousHealth + hydraIncrease);
            }
    ...
    @Test
    @DisplayName("Test basic health calculation hydra - hydra loses")
    public void testRoundCalculationsHydraDie() {
        DungeonManiaController controller = new DungeonManiaController();
        DungeonResponse postBattleResponse = TestUtils.genericHydraSequence(controller,
                "c_battleTests_basicHydraHydraDies");
        BattleResponse battle = postBattleResponse.getBattles().get(0);
        assertBattleCalculations(battle, true, "c_battleTests_basicHydraHydraDies", "hydra");
    }

    @Test
    @DisplayName("Test basic health calculation hydra - player loses")
    public void testRoundCalculationsHydraLive() {
        DungeonManiaController controller = new DungeonManiaController();
        DungeonResponse postBattleResponse = TestUtils.genericHydraSequence(controller,
                "c_battleTests_basicHydraHydraLives");
        BattleResponse battle = postBattleResponse.getBattles().get(0);
        assertBattleCalculations(battle, false, "c_battleTests_basicHydraHydraLives", "hydra");
        assertTrue(compareHydraPreviousHealth(battle, "c_battleTests_basicHydraHydraLives", "hydra"));
    }
    ...
    @Test
    @DisplayName("Test that midnight_armour increases defence and attack of the player")
    public void testPlayerBoostWtihMidnightArmour() {
        DungeonManiaController dmc = new DungeonManiaController();
        DungeonResponse res = dmc.newGame(
                "d_battleTest_midnightArmourBoosts", "c_battleTests_midnightArmourBoosts");

        String config = "c_battleTests_midnightArmourBoosts";
        ...
        List<BattleResponse> battles = res.getBattles();
        BattleResponse battle = battles.get(0);
        // Check that the defence and attack of the player has increased
