...
public class BattleStatistics {
    ...
    public static List<BattleRound> battle(BattleStatistics self, BattleStatistics target, double hydraIncrease) {
        ...
        while (self.getHealth() > 0 && target.getHealth() > 0) {
            double damageOnSelf = target.getMagnifier() * (target.getAttack() - self.getDefence())
                    / self.getReducer();
            double damageOnTarget = self.getMagnifier() * (self.getAttack() - target.getDefence())
                    / target.getReducer();

            self.setHealth(self.getHealth() - damageOnSelf);
            if (hydraIncrease != 0.0) {
                target.setHealth(target.getHealth() + hydraIncrease);
                rounds.add(new BattleRound(-damageOnSelf, hydraIncrease));
            } else {
