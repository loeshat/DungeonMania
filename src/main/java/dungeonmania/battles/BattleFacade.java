...
public class BattleFacade {
    ...
    private boolean willHydraProc(double increaseRate) {
        Random rand = new Random();
        int randInt = rand.nextInt(100);
        double bound = increaseRate * 100;

        return (randInt + 1 <= bound);
    }

    public double hydraIncreaseHealthAmount(Hydra hydra) {
        if (willHydraProc(hydra.getHealthIncreaseRate())) {
            return hydra.getHealthIncreaseAmount();
        }
        return 0.0;
    }
    ...
    public void battle(Game game, Player player, Enemy enemy) {
        ...
        if (enemy instanceof Hydra) {
            increaseAmount = hydraIncreaseHealthAmount((Hydra) enemy);
        }
        ...
        if (increaseAmount != 0.0) {
            enemy.setHealth(enemyBattleStatistics.getHealth() + increaseAmount);
        } else {
